...
function SearchPageView() {
    ...
    const tagFrequence = filterMostFrequent(tagArray, true)
    ...
    if (checkedTagItems.length > 0) {
        ...
        filterData.forEach(item => {
            item.tags = prioritizeTags(checkedTagItems as string[], item.tags)
        })
    ...
    return (
        <Flex wrap="wrap" direction={{ base: 'column', md: 'row' }} mt={14}>
            ...
            <Flex flex={7} wrap="wrap" height="min-content">
                <AnimatePresence >
                    {filterData.map((item) => (
                        <MotionCard key={item.title}
                            maxW='sm'
                            className="hover:shadow-lg transition-shadow duration-300 mb-a"
                        ...
                        >
                            ...
                            <CardBody>
                                {item.tags.map((title) => (
                                    <MotionTag size='sm' key={title} variant='solid'
